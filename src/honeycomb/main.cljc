(ns honeycomb.main
  "Kotodama WASM entrypoint for the Honeycomb clean-room actor (L4) — Clojure port.
  ...
  facts (`honeycomb.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "honeycomb")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"honeycomb.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "honeycomb-compat" :tier tier :entities entities} 200])
...
(defn start! [] :honeycomb-compat/ready)
